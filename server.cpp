...
void add(ffe_t x1, ffe_t y1, ffe_t x2, ffe_t y2, ffe_t
& xR, ffe_t & yR) const
{ if (x1 == 0&& y1 == 0)
{
xR = x2;
...
ec_(&EllipticCurve)
{}
Point(const ffe_t& x, const ffe_t& y, EllipticCurve<P> &
EllipticCurve)
: x_(x),
y_(y),
ec_(&EllipticCurve)
...
template<int T>
ostream& operator <<(ostream& os, const EllipticCurve<T>&
EllipticCurve)
{
...
m1=d*10+c;
cout<<"\n\n";
cout<<"Netra's private key is a = "<<a<<endl;
ec_t::Point Pa = a*G;
cout << "Netra's public key Pa = " << a << "*" << G << " = " << Pa <<
endl<<"\n";
int b = irand(1,myEllipticCurve.Degree()-1);;
ec_t::Point Pb = b*G;
cout << "Shaurya's public key Pb = " << b << "*" << G << " = " << Pb <<
endl<<"\n";
int j = irand(1,myEllipticCurve.Degree()-1);;
ec_t::Point Pj = j*G;
cout << "Thukila's public key Pj = " << j << "*" << G << " = " << Pj << endl;
cout << "\n\n";
cout << "Plain text message from Netra to Shaurya: (" << m1 << ", " << m2
<< ")\n";
ec_t::Point Pk = a*Pb;
cout << "Shared secret between Netra and Shaurya Pk = " << a << "*" << Pb
<<" = " << Pk << endl;
ec_t::ffe_t c1( m1*Pk.x() );
ec_t::ffe_t c2( m2*Pk.y() );
cout << "Encrypted message from Netra to Shaurya = (m.x*Pk.x, m.y*Pk.y) =" <<"(" << c1 << ",
" << c2 << ")\n\n";
cout << "Netra sends Shaurya = {Pa,(c1,c2)} = {" << Pa << ", (" << c1 <<", " << c2 << ")}\n\n";
Pk = b*Pa;
...
{
cout << "Thukila's (the eavesdropper) private key Pj = " << j<< endl;
cout << "\nThukila's decrypted message from Shaurya is = (" << m1d << ", "<< m2d << ")" <<
endl;
